...
def create_metadata(filename, episode_code, title_part, part_num):
    ...
    # Create valid identifier (max 100 chars, valid characters only)
    # Extract season and episode numbers (handle spaces)
    import re
    ...
    if match:
        ...
        ep_clean = re.sub(r'[^a-zA-Z0-9]', '', episode_code.lower())
        identifier = f"nnebula-{ep_clean[:20]}-p{part_num}"
...
def upload_episode(file_path, delay=2):
    ...
    try:
        ...
        print(f"DEBUG: Identifier length: {len(identifier)}")
